...
fun triArea(p0: Vector3, p1: Vector3, p2: Vector3): Double = (p1 - p0).cross(p2 - p0).length() * 0.5
...
fun <T> (Iterable<Pair<T, Double>>).weightedAverage(reference: Vector3, contributionFn: (T) -> Double): Double {
...
fun <T, U> (Iterable<T>).toRTree(dimensions: Int = 3, getFn: (T) -> Pair<Point, U>): RTree<U, Point> = RTree.star().dimensions(dimensions).create<U, Point>().add(
    this.map {
...
data class GeoPoint(val latitude: Double, val longitude: Double) {
    ...
    fun toVector3(): Vector3 = Vector3(
        cos(longitude) * cos(latitude),
        sin(latitude),
        sin(longitude) * cos(latitude)
    )

    fun toVector2(): Vector2 = Vector2(
        longitude / (PI * 2),
        latitude / PI
    )
    ...
    fun formatDigits(digits: Int = 2): String {
        ...
        val eastWest = if (longitudeDegrees >= 0) "W" else "E"
    ...
    companion object {
        fun fromDegrees(latitudeDegrees: Double, longitudeDegrees: Double): GeoPoint = GeoPoint(
