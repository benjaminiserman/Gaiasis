...
object ClimateSimulationGlobals {
    ...
    var itczPathfindingNowVsAnnualInsolationLerp = 0.65
    var itczPathfindingContinentialityWeight = 0.025
    ...
    var airPressureSolarDeclinationScalar = -0.2 // -0.85
    ...
    var warmCurrentAirPressureMaxDistance = 6.0
    ...
    var coolCurrentAirPressureMaxContinentiality = 4.0
    ...
    var coolCurrentAirPressureStrength = 10.0 // maximum impact of cool currents on air pressure in mb
    ...
    var itczAirPressureStrength = -10.5 // maximum impact of the ITCZ on air pressure in mb
    var itczAirPressureMaxDistance = 20.5 // max distance (in tiles) at which the ITCZ effects air pressure

    // WIND

    var windBlockingSlope = 1000.0
    var maxWindBlocking = 0.875
    var backwardsWind = 0.02

    // TEMPERATURE

    var landNowVsAnnualInsolationLerp = 0.15
    var landNowVsAnnualInsolationLerpPow = 1.2

    var baseTemperature = 244.0 // °K, base temperature of land
    var baseTemperatureInsolationScalar = 86.0 // °K, increasing by 1°K ≈ 0.9°K increase in tropical summer temperatures

    var oceanMinBaseTemp = 244.5 // °K, lowest possible ocean temperature
    var oceanBaseTemp = 239.0 // °K, increasing by 1°K ≈ 1°K increase in global ocean temperatures
    var oceanNowVsAnnualInsolationLerp = 0.66 // ∈[0,1], higher values -> less seasonality
    var oceanNowVsAnnualInsolationLerpPow = 0.6 // higher exp -> less insolation & more seasonality
    var oceanInsolationScale = 69.0 // °K, increasing by 1°K ≈ 1°K increase in tropical ocean temperatures
    ...
    var shoreWaterVsLandTemperatureLerpMin = 0.2 // min lerp like above but for shorelines, modulated by peninsularity
    var shoreWaterVsLandTemperatureLerpMax = 0.5 // max lerp like above but for shorelines, modulated by peninsularity
    var inlandWaterVsLandTemperatureContinentialityScalar = 0.06 // lerp scalar for inland tiles based on continentiality
    var inlandWaterVsLandTemperatureContinentialityScalarMax = 4.0
    ...
    var warmCurrentTemperatureStrength = 0.175 // °K, the max temperature increase from a warm current
    var warmCurrentTemperatureDistance = 5.0 // the furthest away that a warm current can affect temperature (in tiles)
    var warmCurrentTemperatureAverageInsolationExp = -1.0
    var warmCurrentTemperatureInsolationExp = -0.4
    var coolCurrentTemperatureStrength = 0.0 // °K, the max temperature decrease from a cool current
    ...
    var moistureCoolingExp = 1.0 // higher exp -> less moisture cooling & faster moisture cooling drop-off
    ...
    var oceanMoistureInsolationScalar = 0.41
    ...
    var ferrelMoistureEffectScalar = 0.175 // exponential falloff rate for ferrel moisture effect
    ...
    var hadleyMoistureEffectScalar = -4.5 // exponential falloff rate for hadley moisture effect
    ...
    var warmCurrentMoistureStrength = 100.0 // °K, the max moisture increase from a warm current
    var warmCurrentMoistureDistance = 7.5 // the furthest away that a warm current can affect moisture
    var warmCurrentMoistureAverageInsolationExp = 9.25
    var coolCurrentMoistureStrength = -40.0 // °K, the max moisture decrease from a cool current
    ...
    var maxMoisturePropagationScale = 1.078
}
